# Chapter 10 Example code and packets

在以下示例中，`ret` 被称为 uninferable；只有在 implicit-return mode 关闭时，这才成立。

## 1. Call to `debug_printf()`，从 `main()` 中的 `80001a84` 发起
...
`jal` 的 target 是 inferable，因此不发送 `te_inst` packet。
...
`ret` 的 target 是 uninferable，因此会发送一个 `te_inst` packet：
...
## 3. 从 `Func_2()` 退出，最后一个 taken branch 后跟一个 `ret`
...
`ret` 的 target 是 uninferable，因此发送一个 `te_inst` packet，其中 `branch_map` 中有 ONE branch：

```text
te_inst[ format=1 (DIFF_DELTA): branches=1, branch_map=0x0, address=0x80001b8a
(delta=0xab0) updiscon=0 ]
...
## 4. 3 个 branches，然后 function return 回到 `Proc_1()`
...
`ret` 的 target 是 uninferable，因此发送一个 `te_inst` packet，其中 `branch_map` 中有 THREE branches：

```text
te_inst[ format=1 (DIFF_DELTA): branches=3, branch_map=0x3, address=0x80001258
(delta=0x148), updiscon=0 ]
```

## 5. 包含 2 个 branches、2 个 `jal` 和一个 `ret` 的复杂示例
...
`jal` 的 target 是 inferable，因此不需要发送 `te_inst` packet。
...
`ret` 的 target 是 uninferable，因此稍后将发送一个 `te_inst` packet。
...
(delta=0xfffffffffffffef4), updiscon=1 ]
```
