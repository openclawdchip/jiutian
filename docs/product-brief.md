...
- Agent 执行面：作为 Always-On 守护层，低功耗运行后台心跳、环境监听、任务队列、上下文投影和经过授权的 Agent 生成代码片段。
- 控制面：作为按需唤醒的高熵任务协处理层，运行操作系统、LLM 推理、编译、重负载软件、安全审计和最终副作用提交。

这种设计允许九天同时保留生态入口和原生效率。

九天把这种协作称为 Guardian-Execution（守护者-执行者）模式：Clawd-Agent 维持机器最低生命体征，Clawd-Super 只在高熵任务出现时被唤醒并高性能爆发。
...
控制面高性能核心，负责系统软件、LLM 推理、编译、重负载计算、安全审计、异常处理和最终副作用提交。它是九天进入现有软件生态的入口，也是高熵任务的战略协处理器；在能效优先配置中应支持 power-gated 或 clock-gated 休眠。

### Clawd-Agent

Agent 执行面核心，负责运行 Agent ISA 指令，并承担 Always-On 守护职责。它优先追求高执行密度、低控制开销、显式数据搬运、低功耗监听和 `WAKE_UP_SUPER` 唤醒能力。
...
守护者-执行者模式。Agent Core 常驻处理低功耗维护与意图分类；当任务被判定为高熵请求时，通过 handoff mailbox 和 `WAKE_UP_SUPER` 唤醒 Super Core。Super Core 完成复杂任务后写回结果并回到休眠，从而扩大动态功耗范围并降低长期热压力。
...
- 不让生成代码绕过安全边界。
- 不让 Super Core 被低价值后台任务长期占用，而是把它定位为高熵任务战略协处理器。
- 不用不可解释的性能主张替代可复现实验。
