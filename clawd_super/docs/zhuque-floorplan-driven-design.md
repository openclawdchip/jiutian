...
- reclaim 返回口靠近 commit，避免旧 PRF 编号跨整个后端折返。
...
- result merge 应允许按簇局部打包，而不是所有来源同时进一个中心 mux。
...
从 floorplan 角度，朱雀后续所有文档都应默认以下组织，不再把它们当作可选优化：
